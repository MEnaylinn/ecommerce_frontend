...
export const fetchAllCartItem=createAsyncThunk("fetchAllCartItem",async(token)=>{
...
export const addCartItem = createAsyncThunk("addCartItem",async(data)=>{
    ...
    try {
        const response = await axios.post(`${cartPath}/add/${data.productId}/${data.username}`,{
            headers : {
...
export const deleteCartItem = createAsyncThunk("deleteCartItem",async(data) => {
    try{
    const response=await axios.delete(`${cartPath}/${data.cartItemId}/delete`,{
        headers : {
            'Authorization' : data.token
        }
    })
...
export const updateCartItem = createAsyncThunk("updateCartItem",async(data) => {
    ...
    try{
    const response=await axios.patch(`${cartPath}/update/${data.cartItemId}/${data.quantity}`,{
        headers : {
            'Content-Type': 'application/json',
            'Authorization' : data.token
        }
    ...
    extraReducers (builder){
        ...
        .addCase(deleteCartItem.fulfilled,(state,action)=>{
            ...
            if(statusCode === 200){
                ...
                const carts = state.cartItems.filter(cart => cart.cartItemId !== Number(data))
        ...
        .addCase(updateCartItem.fulfilled,(state,action)=>{
            ...
            if(statusCode === 200){
                ...
                console.log(data)
                const items = state.cartItems.filter( cartItem => cartItem.id !== Number(data.id))
